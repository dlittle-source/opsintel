...
function RiskRadarCard({ item }: { item: ExecutiveRiskRadarItem }) {
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5 shadow-xl backdrop-blur-sm">
      <div className="flex items-start justify-between gap-4">
        ...
        <div
          className={`rounded-full border px-3 py-1 text-xs font-medium capitalize ${severityStyles[item.severity]}`}
        >
          {item.severity}
        </div>
      </div>
...
function AdvisoryCard({ insight }: { insight: ExecutiveAdvisoryInsight }) {
  return (
    <div className="rounded-2xl border border-slate-800 bg-gradient-to-br from-slate-900 to-slate-950 p-5 shadow-2xl">
      <div className="flex items-center justify-between">
        ...
        <div
          className={`rounded-full border px-3 py-1 text-xs font-medium capitalize ${severityStyles[insight.priority]}`}
        >
          {insight.priority}
        </div>
      </div>
      ...
      <div className="mt-5 border-t border-slate-800 pt-4">
        ...
        <p className="mt-2 text-sm text-slate-200">{insight.action}</p>
      </div>
    </div>
  );
}

export default function EnterpriseOperationsConsole({ data }: Props) {
  const scorecards = [
    {
  ...
  return (
    <section className="space-y-8">
      <div className="relative overflow-hidden rounded-3xl border border-cyan-500/20 bg-gradient-to-br from-slate-950 via-slate-900 to-cyan-950/30 p-8 shadow-[0_0_80px_rgba(6,182,212,0.08)]">
        ...
        <div className="relative z-10">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="rounded-3xl border border-cyan-500/20 bg-black/30 p-6 backdrop-blur-md">
              ...
              <div
                className={`mt-4 inline-flex rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-wider ${severityStyles[data.scorecard.riskLevel]}`}
              >
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            {scorecards.map((card) => (
              <div
                key={card.label}
                className="rounded-2xl border border-slate-800 bg-black/20 p-5 backdrop-blur-sm"
              >
        ...
        <div className="rounded-3xl border border-slate-800 bg-slate-950/70 p-6 shadow-2xl">
          ...
          <div className="mt-6 grid gap-5">
            {data.riskRadar.map((item) => (
              <RiskRadarCard key={item.id} item={item} />
            ))}
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-5">
            {data.advisoryInsights.map((insight) => (
              <AdvisoryCard key={insight.id} insight={insight} />
            ))}
          </div>
        </div>
      </div>
